...
(async () => {
  await Promise.mapSeries(client.listDevices(), async function looper(device) {
    ...
    if(query1) {
      ...
      await query1.tap(null, null, { class: 'android.widget.EditText' });
      await query1.type('한글\n');
    }

    await page.waitFor('.total_wrap');
    const anchor = _.sample(await page.$$('.total_wrap'));
    if(anchor) {
      const anchorText = await anchor.$eval('a', (e) => e.innerText);
      console.log(anchorText);
      await Promise.delay(1000);
      await anchor.tap(null, null, { text: anchorText });
    }
