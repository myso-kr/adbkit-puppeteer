      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 9:
            ...
            ui_bnds = /\[([\d]+),([\d]+)\]\[([\d]+),([\d]+)\]/g.exec(ui_view.bounds);
            ui_rect = _lodash2.default.zipObject(['x1', 'y1', 'x2', 'y2'], [parseInt(ui_bnds[1]), parseInt(ui_bnds[2]), parseInt(ui_bnds[3]), parseInt(ui_bnds[4])]);
            dx1 = Math.floor(x1 * screen_viewport.deviceScaleFactor);
            dy1 = Math.floor(y1 * screen_viewport.deviceScaleFactor);
            dx2 = Math.floor(x2 * screen_viewport.deviceScaleFactor);
            dy2 = Math.floor(y2 * screen_viewport.deviceScaleFactor);
